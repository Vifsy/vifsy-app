-- Spreelo v143.82
-- Plan entitlements from v143.81 plus unlimited capacity for Vercel-configured Spreelo admins.
-- /api/admin/me synchronizes the Vercel admin decision to auth.users app_metadata.spreelo_admin.
-- Customer limits remain enforced by database triggers.
...
comment on function public.spreelo_is_plan_limit_admin(uuid) is
  'v143.82: database-side bypass for users synchronized as Spreelo admins from Vercel admin email configuration.';
