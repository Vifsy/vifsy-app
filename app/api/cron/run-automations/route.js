...
const CAMPAIGN_CONTEXTUAL_PRODUCT_FIT_SCORE = 80;
...
function getCarouselProductConfidence(item) {
  ...
  if (item?.page_type && String(item.page_type).toLowerCase() !== "product") {
    return 0;
  }
...
function selectCarouselProductsFromPool({
  ...
  allowReuseWhenExhausted = STRICT_PRODUCT_NO_REUSE ? false : allowReuseWhenExhausted;
...
function buildRecentUsedCampaignDeliveryItems(_recentUsedItems, _rule, _sourceUrl) {
  // v113: previously used products are never reintroduced as a delivery fallback.
  return [];
}
...
async function prepareCarouselProductsForRule({
  ...
  if (isCampaignRule) {
    ...
    if (hasProductPreparationBudget(90_000)) {
      const storeMapNodes = await getWebsiteStoreMapNodes({
        supabase,
        userId: rule.user_id,
        brandProfileId: rule.brand_profile_id,
        originUrl: getStoreOriginUrl(websiteUrl),
        limit: 80,
      });
      try {
        const marketingStrategy =
          await generateCampaignCarouselMarketingStrategy({
            openai,
            rule,
            brandProfile,
            websiteUrl,
        ...
        rule = applyCampaignMarketingStrategyToRule(
          rule,
          marketingStrategy
        );
      } catch (error) {
        console.warn("Campaign senior strategy unavailable within its bounded budget; continuing with the existing campaign contract", {
          ruleId: rule?.id,
          brandProfileId: rule?.brand_profile_id,
          websiteUrl,
          message: error?.message,
      ...
      logDeadlineSkip("campaign_marketing_strategy");
    }
  }

  // The fast model expands the senior strategy into retailer-search vocabulary.
    ...
    if (queuedPersistentCandidates.length || catalogItems.length) {
      ...
      if (
        ...
        try {
          ...
          console.warn("Persistent campaign pool fast review timed out or failed; continuing with direct text evidence", {
            ruleId: rule.id,
            brandProfileId: rule.brand_profile_id,
            message: error?.message,
          });
    ...
    try {
      ...
      if (evidenceBackedStoreSearchItems.length) {
        try {
          ...
          console.warn("Campaign store-search fast review timed out or failed; continuing with direct text evidence", {
            ruleId: rule.id,
            brandProfileId: rule.brand_profile_id,
            websiteUrl,
    ...
    try {
      ...
      if (uniqueBackupProductCount < CAROUSEL_PRODUCT_SLIDE_TARGET) {
        try {
          ...
          if (isWebsiteRateLimitError(backupDiscoveryError)) {
            throw backupDiscoveryError;
          }
      ...
      if (isWebsiteRateLimitError(backupError)) {
        throw backupError;
      }
  ...
  if (
    ...
    selectedProducts = expanded.selectedProducts
      .filter(isValidCarouselProduct)
      .slice(0, CAROUSEL_PRODUCT_SLIDE_TARGET);
    productEngineV2ReserveProducts = expanded.reserveProducts;
  }

  if (isCampaignRule) {
    const finalReviewUniverse = dedupeWebsiteItemsByUrlTitleAndImage([
      ...selectedProducts,
      ...productEngineV2ReserveProducts,
      ...lockedCampaignSearchPoolItems,
      ...getCampaignSelectionItems(),
      ...catalogItems,
      ...storeMapAgentResult.products,
    ...
    productEngineV2ReserveProducts = finalReview.reserveProducts;
    campaignFinalReviewDiagnostics = {
      publishable: finalReview.publishable,
      shortlist_count: finalReview.shortlistCount,
      eligible_count: finalReview.eligibleCount,
    ...
    console.log("Campaign product preparation completed within bounded budget", {
      ruleId: rule.id,
      brandProfileId: rule.brand_profile_id,
      websiteUrl,
      elapsedMs: Date.now() - productPreparationStartedAt,
      softDeadlineMs: CAROUSEL_PREPARATION_SOFT_DEADLINE_MS,
      withinSoftDeadline:
        Date.now() <= productPreparationCompletionDeadline,
      finalReviewReserveMs: CAMPAIGN_FINAL_REVIEW_RESERVE_MS,
    });
    ...
    if (
      ...
      console.warn("Campaign carousel stopped by senior final relevance gate", {
        ruleId: rule.id,
        brandProfileId: rule.brand_profile_id,
        websiteUrl,
        shortlistCount: finalReview.shortlistCount,
        eligibleCount: finalReview.eligibleCount,
        selectedCount: selectedProducts.length,
        requiredCount: CAROUSEL_PRODUCT_SLIDE_TARGET,
        publishable: finalReview.publishable,
  ...
  if (
    ...
    await throwIfWebsiteDomainCoolingDown(
      websiteUrl,
      "The website temporarily rate limited carousel product verification."
    );
    if (!hasProductPreparationBudget(20_000)) {
      logDeadlineSkip("controlled_incomplete_exit");
    }
    const technicallyVerifiedProductCount =
      dedupeWebsiteItemsByUrlTitleAndImage([
        ...catalogItems,
        ...storeMapAgentResult.products,
      ]).filter(isValidCarouselProduct).length;
    const failureMessage = `Product Engine V2 exhausted direct store search and the remaining catalog fallbacks. The carousel requires ${CAROUSEL_PRODUCT_SLIDE_TARGET} campaign-suitable products with usable images. ${technicallyVerifiedProductCount} products were technically verified, but only ${selectedProducts.length} passed the mandatory senior marketing curation and final relevance review at score ${CAMPAIGN_FINAL_REVIEW_MIN_SCORE} or higher.`;
    ...
    await recordProductEngineV2Run({
      ...
      sourceUrl: websiteUrl,
      platform:
        selectedProducts[0]?.commerce_platform ||
        storeMapAgentResult.products[0]?.commerce_platform ||
        "generic",
      candidateCount: catalogItems.length,
      inspectedCount: Number(
        storeMapAgentResult?.diagnostics?.verified_product_count ||
          storeMapAgentResult.products.length ||
          0
      ),
      verifiedCount: technicallyVerifiedProductCount,
      selectedProducts,
      reserveProducts: productEngineV2ReserveProducts,
      rejectedCount: Math.max(0, catalogItems.length - selectedProducts.length),
      discoveryMethods: [
        STORE_MAP_PRODUCT_AGENT_ENABLED ? "store_map_product_agent" : null,
        "catalog",
        triedStoreSearchForCampaign ? "store_search" : null,
        campaignFreshDiscoveryAttempts > 0 ? "website_discovery" : null,
        "controlled_rotation",
      ].filter(Boolean),
      metadata: {
        campaign_rule: isCampaignRule,
        required_count: CAROUSEL_PRODUCT_SLIDE_TARGET,
        marketing_strategy: rule?.campaign_marketing_strategy || null,
        final_marketing_curation: campaignFinalReviewDiagnostics,
        partial_products: selectedProducts.map((item) => ({
          title: item?.title || null,
          url: item?.url || null,
          image_url: item?.image_url || null,
          search_method:
        ...
        store_map: storeMapAgentResult.diagnostics,
      },
      status: "incomplete",
    ...
    productError.productEngineDiagnostics = {
      candidateCount: catalogItems.length,
      selectedCount: selectedProducts.length,
      storeMap: storeMapAgentResult.diagnostics,
    };
    throw productError;
  }
  ...
  if (
    PRODUCT_ENGINE_V2_ENABLED &&
    !isCampaignRule &&
    !productEngineV2ReserveProducts.length
  ) {
    const selectedKeys = new Set(
      selectedProducts.map((item) => getCarouselProductSelectionKey(item, websiteUrl)).filter(Boolean)
    );
    productEngineV2ReserveProducts = selectCarouselProductsFromPool({
      items: catalogItems,
      rule,
      sourceUrl: websiteUrl,
      recentUsedItems,
  ...
  await upsertWebsiteProductCatalogItems({
    ...
    brandProfileId: rule.brand_profile_id,
    sourceUrl: websiteUrl,
    items: [...selectedProducts, ...productEngineV2ReserveProducts],
  ...
  await recordProductEngineV2Run({
    ...
    platform: selectedProducts[0]?.commerce_platform || "generic",
    candidateCount: catalogItems.length,
    verifiedCount: catalogItems.filter(isValidCarouselProduct).length,
    selectedProducts,
    reserveProducts: productEngineV2ReserveProducts,
    discoveryMethods: [
      STORE_MAP_PRODUCT_AGENT_ENABLED ? "store_map_product_agent" : null,
    ...
    metadata: {
      campaign_rule: isCampaignRule,
      locked_search_pool: hasLockedCampaignSearchPool,
      required_count: CAROUSEL_PRODUCT_SLIDE_TARGET,
      reserve_target: CAROUSEL_PRODUCT_RESERVE_TARGET,
      marketing_strategy: rule?.campaign_marketing_strategy || null,
      final_marketing_curation: campaignFinalReviewDiagnostics,
  ...
  return {
    websiteItems: selectedProducts,
    websiteReserveItems: productEngineV2ReserveProducts,
    productContentContract: buildProductContentContract(selectedProducts, productEngineV2ReserveProducts),
    websiteItem: selectedProducts[0],
    websiteSourceUrl: websiteUrl,
    websiteCycleNumber: cycleNumber,
    useWebsiteImage: true,
...
function buildAutomationPrompt(rule) {
  ...
  const selectedContractProducts = Array.isArray(productContract?.selected_products)
    ? productContract.selected_products
    : [];
...
- A single-product post must stay about that one selected product. It must not become a multi-product bundle or combo.
- The image material is tied to the same selected product list, so the copy must never advertise products that are not shown.`
    : "";
...
async function recordProductEngineV2Run({
  supabase,
  rule,
  sourceUrl,
  platform = "generic",
  ...
  if (!PRODUCT_ENGINE_V2_ENABLED || !rule?.user_id || !rule?.brand_profile_id) {
    return;
  }

  const reusedCount = (selectedProducts || []).filter(
    (item) => item?.campaign_was_used_recently || item?.campaign_image_used_this_run || hasWebsiteItemCatalogUsage(item)
  ).length;
  ...
  try {
    ...
    if (error && error.code !== "42P01") {
      console.log("Product Engine V2 run diagnostics could not be stored", {
        ruleId: rule?.id,
        message: error.message,
        code: error.code,
      });
    ...
    console.log("Product Engine V2 run diagnostics unavailable", {
      ruleId: rule?.id,
      message: error.message,
    });
  }
...
function normalizeWebsiteCatalogItem(row) {
  ...
  return {
    ...
    store_map_metadata: row.store_map_metadata || {},
  };
}
...
async function getWebsiteProductCatalogItems({
  ...
  let { data, error } = await runQuery(WEBSITE_PRODUCT_CATALOG_V3_SELECT);

  if (error && isMissingProductCatalogV2ColumnError(error)) {
    console.warn("Store Map catalog columns are not installed yet; using Product Engine V2 catalog columns", {
      brandProfileId,
      sourceUrl,
      message: error.message,
    });
    ({ data, error } = await runQuery(WEBSITE_PRODUCT_CATALOG_V2_SELECT));
  }

  if (error && isMissingProductCatalogV2ColumnError(error)) {
    console.warn("Product Engine V2 catalog migration is not installed yet; using legacy catalog columns", {
      brandProfileId,
      sourceUrl,
      message: error.message,
    });
    ({ data, error } = await runQuery(WEBSITE_PRODUCT_CATALOG_LEGACY_SELECT));
  }
...
async function getWebsiteProductCatalogItemsByCampaignTerms({
  ...
  for (const term of usableTerms) {
    const pattern = `%${term}%`;
    const runTermQuery = (selectColumns, includeV2Fields = false) =>
      supabase
        .from("website_product_catalog")
        .select(selectColumns)
        .eq("user_id", userId)
        ...
        .or(
          includeV2Fields
            ? `title.ilike.${pattern},product_url.ilike.${pattern},description.ilike.${pattern},category.ilike.${pattern}`
            : `title.ilike.${pattern},product_url.ilike.${pattern},description.ilike.${pattern}`
        )
        .order("times_used", { ascending: true })
        .order("last_used_at", { ascending: true, nullsFirst: true })
        .limit(limitPerTerm);

    let { data, error } = await runTermQuery(WEBSITE_PRODUCT_CATALOG_V3_SELECT, true);
    if (error && isMissingProductCatalogV2ColumnError(error)) {
      ({ data, error } = await runTermQuery(WEBSITE_PRODUCT_CATALOG_V2_SELECT, true));
    }
    if (error && isMissingProductCatalogV2ColumnError(error)) {
      ({ data, error } = await runTermQuery(WEBSITE_PRODUCT_CATALOG_LEGACY_SELECT, false));
    }

    if (error) {
      console.log("Campaign catalog term search unavailable", {
        brandProfileId,
        sourceUrl,
        term,
        message: error.message,
  ...
  return dedupeWebsiteItemsByUrlTitleAndImage(allItems)
    .filter((item) => !item?.page_type || item.page_type === "product");
}
...
async function upsertWebsiteProductCatalogItems({
  ...
  const rows = (items || [])
    .map((rawItem) => {
      const item = normalizeWebsiteItem(rawItem, sourceUrl);
      if (!item?.url || !item?.title) return null;

      const verificationScore = Math.max(
        Number(rawItem?.product_confidence || 0),
        Number(getCarouselProductConfidence({ ...rawItem, ...item }) || 0)
      );
      ...
      return {
        ...
        is_active: true,
        last_seen_at: nowIso,
        commerce_platform: rawItem?.commerce_platform || "generic",
        page_type: rawItem?.page_type || "product",
        ...
        store_map_node_title: rawItem?.store_map_node_title || null,
        store_map_node_type: rawItem?.store_map_node_type || null,
        category_urls: Array.from(
          new Set(
            [
              ...(Array.isArray(rawItem?.category_urls) ? rawItem.category_urls : []),
              rawItem?.store_map_node_url,
              rawItem?.source_page_url,
            ].filter(Boolean)
          )
        ).slice(0, 30),
        store_map_metadata: {
          ai_reason: rawItem?.store_map_ai_reason || null,
          search_method: rawItem?.automation_search_method || null,
          selected_from_map: Boolean(rawItem?.store_map_node_url),
        },
      };
    })
    .filter(Boolean);
  ...
  let rowsForWrite = rows;
  let { error } = await supabase
    .from("website_product_catalog")
    .upsert(rowsForWrite, { onConflict: "brand_profile_id,product_url" });

  if (error && isMissingProductCatalogV2ColumnError(error)) {
    rowsForWrite = rows.map(toV2Row);
    console.warn("Store Map catalog columns missing during upsert; writing Product Engine V2 columns", {
      brandProfileId,
      sourceUrl,
      count: rows.length,
      message: error.message,
  ...
  if (error && isMissingProductCatalogV2ColumnError(error)) {
    rowsForWrite = rows.map(toLegacyRow);
    console.warn("Product Engine V2 catalog migration missing during upsert; writing legacy columns", {
      brandProfileId,
      sourceUrl,
      count: rows.length,
      message: error.message,
...
Return:
{
  ...
  "product_avoid_terms": ["Nearby but wrong product terms"]
    ...
    { timeout: CAMPAIGN_STRATEGY_TIMEOUT_MS }
  );
...
- You may also include contextually suitable product/category searches from the supplied senior strategy. These searches do not need to repeat the theme word, but they must be specific to the campaign's buying situation and known assortment.
- Never introduce a standalone generic gift, present, gift-card, bestseller or personalization query unless that is explicitly one of the senior strategy's contextual directions.
...
Return:
{
  "theme_contract": {
    ...
    "secondary_context": ["Useful context that must not replace the primary theme"]
  },
...
function getCampaignCoreTitleSegment(value) {
  // The campaign meaning is semantic, not positional. A dash, colon or other
  // delimiter has no privileged meaning and the complete title is retained.
...
function extractCampaignTitleThemeTerms(rule) {
  ...
  for (const candidate of getCampaignTitleCandidates(rule)) {
    const segment = getCampaignCoreTitleSegment(candidate);
    const words = tokenizeSearchText(segment).filter(
      (word) =>
        word.length >= 3 &&
        !/^\d+$/.test(word) &&
    ...
    const primaryWord = words.find((word) => word.length >= 4);
  ...
  return collectUniqueTerms(terms, 8);
}
...
function hasTrustedCampaignStoreSearchSignal(item, rule) {
  const foundByQuery = getCampaignStoreSearchQuery(item);
  const sourceSearchUrl = String(item?.source_search_url || "").trim();

  if (!foundByQuery || !sourceSearchUrl) {
    return false;
  }

  const configuredQueries = normalizeStoreSearchQueries(
    splitStoreSearchQueryLine(rule?.product_search_queries),
    CAMPAIGN_STORE_SEARCH_QUERY_LIMIT
  ).map((query) => normalizeSearchText(query).replace(/[-_]+/g, " ").trim());

  return configuredQueries.some(
    (query) =>
      query &&
      (query === foundByQuery ||
        query.includes(foundByQuery) ||
        foundByQuery.includes(query))
  );
}

function getCampaignProductAcceptanceScore(item, rule) {
  return CAMPAIGN_NEAR_PRODUCT_FIT_SCORE;
}
...
function getSafeCampaignProductCandidates(items, rule) {
    ...
    return preferConcreteCampaignProducts(
      dedupeWebsiteItemsByUrlTitleAndImage(items)
        ...
        .filter((item) => {
          ...
          return (
            !isExplicitCampaignFitRejected(item) &&
            Number(item.campaign_theme_term_matches || 0) > 0 &&
            (aiScore === null ||
        ...
        .sort((left, right) => {
          const evidenceDelta =
            Number(right.campaign_theme_term_matches || 0) -
            Number(left.campaign_theme_term_matches || 0);
          if (evidenceDelta !== 0) return evidenceDelta;

          const aiDelta =
            Number(getAiCampaignFitScore(right) ?? -1) -
            Number(getAiCampaignFitScore(left) ?? -1);
  ...
  const dedupedItems = dedupeWebsiteItemsByUrlTitleAndImage(items).filter((item) => {
    ...
    // A concrete product verified from a campaign-created retailer search is
    // already backed by useful retrieval context. Keep medium product-level
    // matches available instead of requiring every title to name the occasion.
    return aiScore === null || aiScore >= getCampaignProductAcceptanceScore(item, rule);
  });
  const explicitTerms = extractExplicitCampaignMatchTerms(rule);
  const anchorTerms = extractCampaignAnchorTerms(rule);
...
function getCampaignProductSignalState(
  ...
  const directMatches = hasCoreThemeGuard
    ? themeMatches
    : hasAnchorGuard
      ? anchorMatches
      : primaryMatches;
  const relevance = evaluateCampaignProductRelevance({
    directMatches,
    aiScore: aiCampaignFitScore,
    verdict: item?.campaign_fit_verdict,
    relevanceClass: item?.campaign_relevance_class,
    campaignRole: item?.campaign_role,
    evidence:
      item?.campaign_relevance_evidence ||
      item?.campaign_final_review_evidence,
    reason: item?.campaign_fit_reason,
    directMinimumScore: effectiveMinimumAiScore,
    contextualMinimumScore: CAMPAIGN_CONTEXTUAL_PRODUCT_FIT_SCORE,
  });
  ...
  return {
    ...
    contextualCampaignApproval: relevance.contextualApproved,
    campaignRole: relevance.campaignRole,
    campaignRelevanceEvidence: relevance.evidence,
...
- Named occasion priority: if the campaign is built around a specific holiday, season, event, theme day or cultural occasion, direct occasion products should beat generic gifts, personalized products, bestsellers, pet portraits, custom-print items or broad category items.
- Do not require a naturally suitable contextual product to repeat the occasion word in its title. It may be approved only when its verified facts support a specific role in this exact campaign.
- Classify each candidate as direct, contextual, generic or reject. Contextual means a natural, campaign-specific buying/use-case fit. Generic giftability, popularity, attractiveness or personalization alone is never contextual.
- For every contextual candidate, name its campaign role and cite concrete supplied product facts. Without both, cap it below 75.
- Generic gift or personalized products must remain below 75 unless they also have direct evidence.
...
Senior marketing strategy:
${formatCampaignMarketingStrategyForPrompt(rule?.campaign_marketing_strategy) || "No structured strategy was available."}
  ...
  "scores": [
    {
      ...
      "campaign_role": "Specific role this product serves in the campaign, or empty",
      "evidence": ["Supplied product facts that justify the class and role"],
      "reason": "Short reason"
    }
  ]
}
    ...
    for (const entry of scores) {
      ...
      const evaluation = {
        ...
        evidence: normalizeCampaignRelevanceEvidence(entry?.evidence, 8),
  ...
  return items.map((item, index) => {
    ...
    const campaignFitEvaluations = [...priorEvaluations, ...currentEvaluations]
      ...
      .map((entry) => ({
        ...
        verdict: String(entry.verdict || "").trim(),
        relevanceClass: normalizeCampaignRelevanceClass(
          entry.relevanceClass || entry.relevance_class
        ),
        campaignRole: normalizeCampaignStrategyText(
          entry.campaignRole || entry.campaign_role,
          220
        ),
        evidence: normalizeCampaignRelevanceEvidence(entry.evidence, 8),
        reason: String(entry.reason || "").trim(),
    ...
    return {
      ...
      campaign_relevance_class:
        evaluation.relevanceClass ||
        item?.campaign_relevance_class ||
        null,
      campaign_role:
...
function selectCampaignFitEscalationCandidates({
  candidates,
  evaluationByIndex,
  rule,
  minimumStrongProducts,
  escalationMaxItems,
}) {
  const evaluated = candidates.map((candidate) => {
    const evaluation = evaluationByIndex.get(candidate.index);
    const score = evaluation ? Number(evaluation.score || 0) : null;
    const directMatches = Math.max(
      countCampaignCoreThemeTermMatches(candidate.item, rule),
      countCampaignAnchorTermMatches(candidate.item, rule),
      countPrimaryCampaignTermMatches(candidate.item, rule)
    );
  ...
  const strongCount = evaluated.filter(
    (candidate) => Number(candidate.currentScore || 0) >= CAMPAIGN_STRONG_PRODUCT_FIT_SCORE
  ).length;
  const missingStrongCount = Math.max(0, minimumStrongProducts - strongCount);
  const desiredEscalationCount = Math.min(
    escalationMaxItems,
    Math.max(4, missingStrongCount * 2 + 2)
  );

  return evaluated
    .filter((candidate) => {
      if (candidate.currentScore === null) return true;
      if (candidate.currentScore >= 45 && candidate.currentScore < 90) return true;
      return candidate.directMatches > 0 && candidate.currentScore < CAMPAIGN_STRONG_PRODUCT_FIT_SCORE;
    })
    .sort((a, b) => {
      const directDelta = Number(b.directMatches || 0) - Number(a.directMatches || 0);
    ...
    .slice(0, desiredEscalationCount);
}
...
async function applyAiCampaignFitScores({
  ...
  const candidates = items
    ...
    .sort((a, b) => {
      const evidenceDelta =
        countCampaignCoreThemeTermMatches(b.item, rule) -
        countCampaignCoreThemeTermMatches(a.item, rule);
      if (evidenceDelta !== 0) return evidenceDelta;
  ...
  if (
    ...
    const escalationCandidates = selectCampaignFitEscalationCandidates({
      candidates,
      evaluationByIndex,
      rule,
      minimumStrongProducts,
...
function getCampaignFinalReviewGroupKey(item, index) {
  ...
  return `item:${getCarouselProductSelectionKey(item) || index}`;
}

function countCampaignTitleThemeEvidence(item, rule) {
  const titleThemeTerms = extractCampaignTitleThemeTerms(rule);
  if (!titleThemeTerms.length) return 0;
  ...
  const ranked = dedupeWebsiteItemsByUrlTitleAndImage(items)
    ...
    .map((item, index) => {
      ...
      const signalState = getCampaignProductSignalState(
        item,
        rule,
        CAMPAIGN_FINAL_REVIEW_MIN_SCORE
      );
      return {
        item,
        index,
...
function buildBoundedCampaignFinalReviewFallback({
  ...
  const evidenceBackedItems = shortlist
    ...
    .map((item) => ({
      ...
      campaign_fit_source:
        item?.campaign_fit_source || "bounded_text_evidence_fallback",
      campaign_fit_reason:
        item?.campaign_fit_reason ||
        "Selected from verified direct or auditable contextual campaign evidence after the bounded final review was unavailable.",
    }));
...
function getCampaignFinalEvaluationRelevance(item, evaluation, rule) {
  ...
  return evaluateCampaignProductRelevance({
    directMatches,
    ...
    evidence: evaluation?.evidence,
    reason: evaluation?.reason,
    directMinimumScore: CAMPAIGN_FINAL_REVIEW_MIN_SCORE,
    contextualMinimumScore: CAMPAIGN_CONTEXTUAL_PRODUCT_FIT_SCORE,
  });
}
...
- When a named theme exists and the shortlist contains at least ${selectedLimit} products with direct product evidence for it, the majority of the selected set must come from those theme-evidenced products. Generic gifts, gift cards, bestsellers, personalization and broadly attractive products are fallback material, not substitutes for better theme products.
- Theme evidence may come from the supplied product title, description, category, tags or product URL. The retailer query and result page are discovery provenance only and never evidence.
- A product may also be contextually suitable without repeating the theme word. Classify it as contextual only when verified product facts support a specific, natural role in this campaign and in the coherent five-product set.
- "Giftable", "popular", "attractive", "personalized" or "a bestseller" alone is generic, not contextual. Generic candidates are not publication-eligible.
- Return relevance_class, campaign_role and concrete evidence for every candidate. A contextual candidate without a specific role and evidence must be rejected.
- Evaluate the product itself. A retailer search query is only a discovery clue and is never proof that the product has the searched property.
- Every selected item must be a concrete verified product page and score at least ${CAMPAIGN_FINAL_REVIEW_MIN_SCORE}/100 for this exact campaign and its intended slot.
...
- Select exactly ${selectedLimit} products only when the complete set is genuinely publication-ready. Otherwise set publishable to false and select only the defensible products.
- Do not silently fill an uncovered strategy slot with the next-highest standalone score.
- Mark a missing need as blocking only when it truly prevents the selected five-product set from being published. Optional ways to improve an already complete set must be non-blocking.
...
Return strict JSON only:
{
  ...
  "evaluations": [
    {
      "index": 0,
      "score": 0,
      "verdict": "excellent | strong | weak | reject",
      "relevance_class": "direct | contextual | generic | reject",
  ...
  "selected_indices": [0, 1, 2, 3, 4]
    ...
    console.warn("Campaign senior final review timed out or failed; using bounded direct text-evidence fallback", {
      ruleId: rule?.id,
      brandProfileId: rule?.brand_profile_id,
      websiteUrl,
      shortlistCount: shortlist.length,
  ...
  const parsed = safeJsonParse(response.output_text || "");
  if (!parsed || typeof parsed !== "object") {
    console.warn("Campaign senior final review returned invalid JSON; using bounded direct text-evidence fallback", {
      ruleId: rule?.id,
      brandProfileId: rule?.brand_profile_id,
      websiteUrl,
      shortlistCount: shortlist.length,
  ...
  if (evaluationByIndex.size < selectedLimit) {
    console.warn("Campaign senior final review returned too few evaluations; using bounded direct text-evidence fallback", {
      ruleId: rule?.id,
      brandProfileId: rule?.brand_profile_id,
      websiteUrl,
      shortlistCount: shortlist.length,
  ...
  if (
    ...
    selectedIndices.length < selectedLimit
  ) {
...
function scoreCampaignFitForRule(item, rule) {
  ...
  const directCampaignSignalCount = hasCoreThemeGuard
    ...
    : hasAnchorGuard
      ? anchorMatches
      : primaryMatches;
  const contextualCampaignApproval = evaluateCampaignProductRelevance({
    directMatches: directCampaignSignalCount,
    aiScore,
    verdict: item?.campaign_fit_verdict,
    relevanceClass: item?.campaign_relevance_class,
    campaignRole: item?.campaign_role,
    evidence:
      ...
      item?.campaign_final_review_evidence,
    reason: item?.campaign_fit_reason,
    directMinimumScore: CAMPAIGN_NEAR_PRODUCT_FIT_SCORE,
    contextualMinimumScore: CAMPAIGN_CONTEXTUAL_PRODUCT_FIT_SCORE,
  }).contextualApproved;
