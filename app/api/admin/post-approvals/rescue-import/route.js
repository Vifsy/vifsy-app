...
function normalizeProduct(raw, index) {
  return {
    ...
    image_file: cleanText(raw?.image_file || raw?.image_filename || raw?.image, 500),
...
export async function POST(request) {
  ...
  try {
    ...
    for (let index = 0; index < normalized.length; index += 1) {
      ...
      if (!product.title || !product.url || !product.image_file) {
        return Response.json({ ok: false, error: `Product ${index + 1} needs product_name/title, product_url and image_file.` }, { status: 400 });
      }
      const imageEntry = findEntry(entries, product.image_file);
      if (!imageEntry) {
        return Response.json({ ok: false, error: `Image file '${product.image_file}' for product ${index + 1} is missing.` }, { status: 400 });
      }
      ...
      const ext = baseName(imageEntry.name).split(".").pop().toLowerCase();
      const contentType = MIME_BY_EXT[ext];
      if (!contentType) {
        return Response.json({ ok: false, error: `Image ${index + 1} must be PNG, JPG or WEBP.` }, { status: 400 });
      }
      try {
        const metadata = await sharp(Buffer.from(imageEntry.bytes), { failOn: "error" }).metadata();
        const format = String(metadata?.format || "").toLowerCase();
        if (!["jpeg", "png", "webp"].includes(format) || !metadata?.width || !metadata?.height) {
          throw new Error("unsupported image data");
        }
        imageMetadata.set(baseName(imageEntry.name).toLowerCase(), { width: metadata.width, height: metadata.height, format });
      } catch {
    ...
    for (let index = 0; index < normalized.length; index += 1) {
      ...
      const imageEntry = findEntry(entries, product.image_file);
      const ext = baseName(imageEntry.name).split(".").pop().toLowerCase();
      const storagePath = `rescue/${workItem.id}/${index + 1}-${crypto.randomUUID()}.${ext === "jpeg" ? "jpg" : ext}`;
      const upload = await context.admin.storage.from(BUCKET).upload(
        storagePath,
        Buffer.from(imageEntry.bytes),
        { contentType: MIME_BY_EXT[ext], upsert: false }
      );
      ...
      importedProducts.push({
        ...
        product_image_width: imageMetadata.get(baseName(imageEntry.name).toLowerCase())?.width || null,
        product_image_height: imageMetadata.get(baseName(imageEntry.name).toLowerCase())?.height || null,
        ...
        manual_override_note: "Imported from ChatGPT rescue ZIP; admin must preview before regeneration.",
      });
    }
