...
const contentTypes = [
  {
    ...
    imagePrompt:
      "Create a premium 4:5 editorial product post around the verified product. If the website image already has real transparency, place that exact product inside the working 4:5 canvas and generate the full finished composition around it in one pass while preserving the product unchanged. If it does not have transparency, skip background-removal attempts and faithfully recreate the exact product inside the finished composition. Give the product generous space, use a product-specific background, and integrate a large mobile-readable headline plus the exact product/model name only. No third copy row, CTA button, price, ratings or invented claims.",
...
const RETIRED_CONTENT_TYPE_IDS = new Set([
  "behind_scenes",
  "case_example",
  "local",
  "comparison",
...
const autoPlanGoals = [
  ...
  },
  {
    ...
    description:
      "Useful, shareable content that gives new people a reason to discover and follow the account.",
  },
  {
    id: "build_trust",
    icon: "🤝",
    label: "Build trust",
    description:
      "Expertise, guidance and clear answers that make the business easier to understand and trust.",
  },
];
...
export default function AutomationPage() {
  ...
  function translateContentTypeLabel(type) {
    ...
    if (type?.id === "website_item") return t("automation.contentType.website_item.labelV127");
  ...
  function translateContentTypeShortLabel(type) {
    ...
    if (type?.id === "website_item") return t("automation.contentType.website_item.shortLabelV127");
  ...
  function translateContentTypeDescription(type) {
    ...
    if (type?.id === "website_item") return t("automation.contentType.website_item.descriptionV127");
  ...
  const plannerSectionCopy = {
    settingsTitle: t("automation.redesign.settingsTitle"),
    settingsText: t("automation.redesign.settingsTextV2"),
    goalHelp: t("automation.redesign.goalCardHelp"),
    frequencyHelp: t("automation.redesign.frequencyCardHelp"),
    startDateHelp: t("automation.redesign.startDateCardHelp"),
    languageHelp: t("automation.redesign.languageCardHelp"),
    publishingHelp: t("automation.redesign.publishingCardHelp"),
    platformHelp: t("automation.redesign.platformCardHelp"),
  };

  const plannerGoalCopy = {
    sell_more: {
      label: t("automation.planGoal.sell_more.labelV127"),
      description: t("automation.planGoal.sell_more.descriptionV127"),
    },
    get_followers: {
      label: t("automation.planGoal.get_followers.labelV127"),
      description: t("automation.planGoal.get_followers.descriptionV127"),
    },
    build_trust: {
      label: t("automation.planGoal.build_trust.labelV127"),
      description: t("automation.planGoal.build_trust.descriptionV127"),
    },
    educate_customers: {
  ...
  const exploreFormatItems = useMemo(() => {
    ...
    return contentFormatLibrary
      ...
      .map((config) => {
        ...
        const isCleanProductPost = formatId === "website_item";
        const isCleanProductCarousel = formatId === "carousel_website_item";
        ...
        return {
          ...
          howItWorks: t(
            isCleanProductPost
              ...
              : isCleanProductCarousel
              ? "automation.formatCard.carousel_website_item.howItWorksV127"
              : `automation.formatCard.${formatId}.howItWorks`
          ),
          benefit: t(
            isCleanProductPost
              ? "automation.formatCard.website_item.benefitV127"
              : `automation.formatCard.${formatId}.benefit`
          ),
        };
      })
      .filter(Boolean)
...
const shouldShowPlannerDetails =
  ...
  async function inspectFocusSource() {
    ...
    try {
      ...
      setFocusSource(null);
      setFocusSourceError(error.message || t("automation.focusSource.verifyFailed"));
    } finally {
  ...
  async function createOfferCampaignPlan() {
    ...
    if (
      ...
      setOfferPlannerError(t("automation.offerPlan.discountError"));
      return;
    }
...
const shouldShowCampaignDebug = searchParams?.get("debugCampaign") === "1";
              ...
              {offerPlannerOpen ? (
                <section className="plan-v70-offer-builder" id="plan-v72-offer-builder">
                  ...
                  <div className="plan-v70-offer-scope">
                    ...
                    >
                      <GalleryHorizontalEnd size={16} /> {t("automation.offerPlan.productCategory")}
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <ShoppingBag size={16} /> {t("automation.offerPlan.singleProduct")}
                    </button>
                  </div>
                  ...
                  <div className="plan-v70-offer-grid">
                    ...
                    <label>
                      <span>{t("automation.offerPlan.discountLabel")}</span>
                    ...
                    <label>
                      <span>{t("automation.offerPlan.discountTypeLabel")}</span>
                      <select value={offerDiscountType} onChange={(event) => setOfferDiscountType(event.target.value)}>
                        <option value="percent">{t("automation.offerPlan.percentageOption")}</option>
                        <option value="amount">{t("automation.offerPlan.amountOption")}</option>
                      </select>
                    </label>
                    {offerDiscountType === "amount" ? (
                      <label>
                        <span>{t("automation.offerPlan.currencyLabel")}</span>
                        ...
                        <small className="plan-v100-currency-note">
                          {t("automation.offerPlan.currencyNote")}
                        </small>
                      </label>
                    ) : null}
                  ...
                  <div className="plan-v70-offer-actions">
                    <p>{t("automation.offerPlan.actionHelp")}</p>
  ...
  >
    {planCreationMode === "campaign"
      ...
      : translateAutoPlanGoalDescription(autoPlanGoal) ||
        t("automation.redesign.chooseGoalPrompt")}
  </SetupHelperNote>
</div>
          ...
          <div className="plan-v83-modal-backdrop" onClick={() => setShowSocialChannelRequiredModal(false)}>
            ...
            >
              ...
              <p className="plan-v83-modal-eyebrow">
                {t("automation.channelRequired.eyebrow")}
              </p>
              <h2 id="plan-v100-channel-required-title">
                {t("automation.channelRequired.title")}
              </h2>
              <p className="plan-v83-modal-lead">
                {t("automation.channelRequired.text")}
              </p>
