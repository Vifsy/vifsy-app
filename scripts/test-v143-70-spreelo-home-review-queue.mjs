...
expect(home.includes('completedAdminStates') && home.includes('approved_by_spreelo'), 'Home must exclude posts already approved by Spreelo from the waiting count.');
expect(home.includes('/admin/post-approvals?view=queue'), 'Home review CTA must open the review queue, not a random post.');
expect(home.includes('/admin/post-approvals?view=history'), 'Home must expose content review history.');
...
expect(uiText.includes('TRANSLATION_CACHE_VERSION = "v14"'), 'Translation cache must be refreshed so newly added labels are fetched in other languages.');
