const USD = "USD";
const PRICE_VERSION = "2026-08-23";
...
const OPENAI_IMAGE_PRICES_PER_MILLION = {
  "gpt-image-2": { textInput: 5, cachedTextInput: 1.25, imageInput: 8, cachedImageInput: 2, imageOutput: 30 },
};
...
export function calculateOpenAICost({ operation, request, response }) {
  ...
  return {
    ...
    requestId,
    pricingVersion: PRICE_VERSION,
    quantity: numberOrZero(usage?.total_tokens) || inputTokens + outputTokens,
    unit: "tokens",
    usage: {
...
export function createGenerationCostTracker({
  ...
  const record = async (rawEvent) => {
    ...
    const payload = {
      ...
      generation_session_id: currentGenerationSessionId,
      generation_user_id: currentGenerationUserId,
      ...event,
    };
    const { error } = await supabase
      .from("post_generation_cost_events")
      .upsert(payload, { onConflict: "provider,provider_request_id,operation", ignoreDuplicates: true });
        ...
        if (typeof value === "function") {
          ...
          return async (...args) => {
            ...
            if (tracker?.recordOpenAI) {
              try {
                await tracker.recordOpenAI(methodPath, args[0] || {}, response);
              } catch (error) {
