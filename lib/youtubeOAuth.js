...
export async function saveYouTubeConnection({
  ...
  // Match Facebook/Instagram/Threads/Pinterest semantics: a provider account can
  // belong to only one brand at a time for the same Spreelo user. If the same
  // YouTube channel is connected from another brand, move the existing row
  // instead of inserting a duplicate user/platform/page_id combination.
  const { data: existing, error: existingError } = await supabaseAdmin
    .from("social_connections")
    .select("id, refresh_token, brand_profile_id")
    .eq("user_id", userId)
    .eq("platform", "youtube")
    .eq("page_id", channelId)
    .maybeSingle();
  if (existingError) throw existingError;

  const durableRefreshToken = refreshToken || existing?.refresh_token || null;
  ...
  // The target brand may currently have another YouTube channel attached.
  // Disconnect it first; the selected channel below is then connected/moved in.
