...
import { getStaticEmailLabels } from "./staticEmailText.js";
...
export async function getOrCreateServerNamespaceLabels({
  ...
  // Customer emails always follow the user's selected app locale. Curated
  // file-backed copy is preferred. For supported locales where the static file
  // does not yet contain every email string, create the missing language pack
  // once globally and persist it in ui_translation_packs. Subsequent sends are
  // cache/file backed and make no AI translation call. This avoids the old
  // per-email translation cost while still preventing a German/French/etc.
  // account from silently receiving English service mail.
  if (namespace === "emails") {
    if (!supabaseAdmin) return getStaticEmailLabels(safeLocale, {});

    const { data: emailPack, error: emailPackError } = await supabaseAdmin
      .from("ui_translation_packs")
      .select("id, labels, status")
      .eq("locale", safeLocale)
      .eq("namespace", namespace)
      .maybeSingle();
    if (emailPackError) throw emailPackError;

    const persistedLabels = emailPack?.labels || {};
    const effectiveLabels = getStaticEmailLabels(safeLocale, persistedLabels);
    const missingLabels = getLabelsNeedingTranslation(defaultLabels, effectiveLabels, safeLocale);
    if (Object.keys(missingLabels).length === 0) return effectiveLabels;
    ...
    const mergedPersistedLabels = { ...persistedLabels, ...translatedMissingLabels };
    const { error: emailUpsertError } = await supabaseAdmin.from("ui_translation_packs").upsert(
      {
        locale: safeLocale,
        language: languageName,
        namespace,
        labels: mergedPersistedLabels,
        status: "ready",
        updated_at: new Date().toISOString(),
      },
      { onConflict: "locale,namespace" }
    ...
    return getStaticEmailLabels(safeLocale, mergedPersistedLabels);
  }
