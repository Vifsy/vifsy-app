...
export async function getOrCreateServerNamespaceLabels({
  ...
  // Transactional/service emails are deliberately file-backed and cost-free at
  // send time. Reuse any already-persisted translations as a compatibility
  // layer, but never call OpenAI for the emails namespace. Missing strings fall
  // back to the curated static pack and finally English source labels.
  if (namespace === "emails") {
    let persistedLabels = {};
    if (supabaseAdmin) {
      const { data: emailPack, error: emailPackError } = await supabaseAdmin
        ...
        .maybeSingle();
      if (!emailPackError && emailPack?.labels) persistedLabels = emailPack.labels;
    }
    return getStaticEmailLabels(safeLocale, persistedLabels);
  }
