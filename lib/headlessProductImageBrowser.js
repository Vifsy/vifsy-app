...
function normalizeCandidate(candidate) {
  ...
  const combined = `${source} ${candidate?.context || ""} ${url}`;
  if (
    !url ||
    !/^https?:\/\//i.test(url) ||
    REVIEW_OR_USER_IMAGE_SIGNAL_PATTERN.test(combined) ||
...
async function collectRenderedCandidates(page) {
  return page.evaluate(() => {
    ...
    const scoreFor = (element, context, width) => {
      ...
      const recommendationRegion = element.closest(
        '[class*="recommend"],[class*="related"],[class*="similar"],' +
          '[class*="recently"],[class*="cross-sell"],[class*="upsell"],' +
          '[class*="review"],[class*="testimonial"],[class*="judgeme"],' +
          '[data-testid*="recommend"],[data-testid*="related"],' +
          '[data-testid*="similar"],[data-testid*="recently"],' +
      ...
      if (displayedArea >= 120_000) {
        score = Math.max(score, 116);
      } else if (displayedArea >= 50_000) {
        score = Math.max(score, 104);
      } else if (displayedArea > 0 && displayedArea <= 12_000) {
        score -= 30;
      }
      if (
        recommendationRegion ||
        /thumb|thumbnail|tablist|swatch|recommend|similar|related|recently|cross.sell|upsell|judgeme|judge.me|loox|yotpo|review.(?:image|photo|media|gallery)|customer.(?:image|photo|review|media)|user.generated|\bugc\b|testimonial/i.test(
          combined
        )
      ) {
        score -= 60;
...
async function clickPrimaryProductImage(page, primaryImageUrl = "") {
  const handle = await page.evaluateHandle((preferredUrl) => {
    const negativePattern =
      /thumb|thumbnail|tablist|swatch|recommend|similar|related|review|testimonial|judgeme|judge.me|loox|yotpo|logo|icon/i;
    let preferredPath = "";
    try {
    ...
    const images = [...document.querySelectorAll("main img,[role=main] img,img")]
      .map((image) => {
        ...
        const displayedArea =
          ...
          Math.max(0, Number(rect.height || 0));
        const naturalArea =
          Math.max(0, Number(image.naturalWidth || 0)) *
          Math.max(0, Number(image.naturalHeight || 0));
        const area = displayedArea * 100 + Math.min(naturalArea, 5_000_000);
        let currentPath = "";
        try {
          currentPath = new URL(
        ...
        const contextWeight = negativePattern.test(context) ? 0.01 : 1;
        return {
          image,
