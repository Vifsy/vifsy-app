...
export function prioritizeStoreMapShelvesForExecution(nodes = []) {
  const scoreShelfExecutionPriority = (node) => {
    const aiScore = Number(node?.store_map_ai_score || 0);
    const deterministicScore = Number(node?.store_map_intent_score || 0);
    let pathDepth = 0;
    try {
      pathDepth = new URL(node?.url || "").pathname.split("/").filter(Boolean).length;
    } catch {
      pathDepth = 0;
    }
    const shelfText = `${node?.title || ""} ${node?.url || ""}`.toLowerCase();
    const broadShelfPenalty =
      /(?:^|[\s/_-])(?:all|view-all|clothing|clothes|apparel|fashion|women|men|kids|products)(?:$|[\s/_-])/i.test(
        shelfText
      )
        ? 35
        : 0;
    const oversizedShelfPenalty =
      Number(node?.product_link_count || 0) >= 2000 ? 30 : 0;
    return (
      aiScore * 2 +
      deterministicScore +
      Math.min(64, pathDepth * 8) -
      broadShelfPenalty -
      oversizedShelfPenalty
    );
  };
  ...
  return (nodes || [])
    .map((node) => ({
      ...node,
      store_map_execution_priority: scoreShelfExecutionPriority(node),
    }))
    .sort(
      (a, b) =>
        Number(b.store_map_execution_priority || 0) -
        Number(a.store_map_execution_priority || 0)
    );
}
...
export function getStoreMapAgentTargets(requiredCount = 5) {
  ...
  return {
    requiredCount: count,
    reserveCount: isCarouselScale ? Math.max(count, 5) : 1,
    minimumVerifiedProducts: isCarouselScale
      ? Math.max(20, count * 4)
      : 1,
    mapCrawlPageLimit: isCarouselScale ? Math.max(12, count * 3) : 6,
    mapDepthLimit: 3,
    shelfSelectionLimit: isCarouselScale
      ? Math.max(4, Math.min(6, count + 1))
      : 2,
    shelfProductLimit: isCarouselScale ? Math.max(20, count * 4) : 6,
    mapNodeLimit: 600,
  };
}
