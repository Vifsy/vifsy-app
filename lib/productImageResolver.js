...
export const PRODUCT_IMAGE_IDENTITY_ANCHOR_MIN_LONG_SIDE = 200;
export const PRODUCT_IMAGE_IDENTITY_ANCHOR_MIN_AREA = 40_000;
...
const REVIEW_OR_USER_IMAGE_SIGNAL_PATTERN =
  /(?:judgeme|judge\.me|loox|yotpo|stamped(?:\.io)?|trustpilot|bazaarvoice|powerreviews|review[-_ /]?(?:image|photo|media|gallery)|customer[-_ /]?(?:image|photo|review|media)|user[-_ ]?generated|\bugc\b|testimonial)/i;

export function isReviewOrUserGeneratedProductImageCandidate(candidate = {}) {
  return REVIEW_OR_USER_IMAGE_SIGNAL_PATTERN.test(
    `${candidate?.source || ""} ${candidate?.context || ""} ${candidate?.url || ""}`
  );
}
...
function parseSrcset(value) {
  return String(value || "")
    // CDN image URLs can contain commas in their query parameters, e.g.
    // `?w=1600,h=1600,quality=85`. Only split at actual srcset separators.
    .split(/,\s+(?=\S)/)
...
function inferDeclaredWidthFromImageUrl(value) {
  try {
    const parsed = new URL(String(value || ""));
    let inferredWidth = 0;

    for (const [key, rawValue] of parsed.searchParams.entries()) {
      if (!RESIZE_QUERY_KEY_PATTERN.test(key)) continue;
      const widthMatch = String(rawValue || "").match(
        /(?:^|[^\d])(\d{2,5})(?:[^\d]|$)/
      );
      inferredWidth = Math.max(
        inferredWidth,
        Number(widthMatch?.[1] || 0)
      );
    }

    const pathWidthMatches = [
      ...parsed.pathname.matchAll(
        /(?:^|[/_-])w[_-]?(\d{2,5})(?=[/_.-]|$)/gi
      ),
      ...parsed.pathname.matchAll(
        /(?:^|[/_-])(\d{2,5})x\d{0,5}(?=[/_.-]|$)/gi
      ),
    ];
    for (const match of pathWidthMatches) {
      inferredWidth = Math.max(inferredWidth, Number(match?.[1] || 0));
    }

    return inferredWidth;
  } catch {
    return 0;
  }
}
...
function inferContextRoleScore({ source, context, url, declaredWidth = 0 }) {
  ...
  if (REVIEW_OR_USER_IMAGE_SIGNAL_PATTERN.test(combined)) {
    score -= 120;
  }
...
export function canonicalProductImageAssetKey(value) {
  try {
    ...
    parsed.pathname = parsed.pathname
      // Drupal image styles are derivatives of the same underlying file. Treat
      // /files/styles/<style>/public/.../photo.jpg.webp as the same asset as
      // /files/.../photo.jpg so a larger original can safely replace the small
      // gallery derivative without weakening product identity.
      .replace(/\/styles\/[^/]+\/public\//i, "/")
      .replace(/\.(?:avif|webp)$/i, (suffix) => {
        const before = parsed.pathname.slice(0, -suffix.length);
        return /\.(?:jpe?g|png|gif)$/i.test(before) ? "" : suffix;
      })
...
export function findSharedProductImageAssetKeys(items = []) {
  const productUrlsByAsset = new Map();
  ...
  for (const item of items || []) {
    ...
    const productUrl = String(
      ...
      .toLowerCase()
      .replace(/\/+$/, "");
    const assetKey = canonicalProductImageAssetKey(imageUrl);

    if (!assetKey || !productUrl) continue;
    if (!productUrlsByAsset.has(assetKey)) {
      productUrlsByAsset.set(assetKey, new Set());
    }
    productUrlsByAsset.get(assetKey).add(productUrl);
  }

  return new Set(
    [...productUrlsByAsset.entries()]
      .filter(([, productUrls]) => productUrls.size > 1)
...
export function generateGenericHighResolutionImageUrls(
  ...
  // Drupal/Saint-Gobain sites often expose a 375-500 px image-style derivative
  // even though the original file is public at the same path without
  // /styles/<style>/public/. Probe that exact same underlying asset before
  // accepting a small fallback. This does not scan recommendations or infer a
  // different product image.
  if (/\/styles\/[^/]+\/public\//i.test(pathname)) {
    const original = new URL(parsed);
    original.pathname = pathname
      .replace(/\/styles\/[^/]+\/public\//i, "/")
      .replace(/\.(?:avif|webp)$/i, (suffix) => {
        const before = original.pathname.slice(0, -suffix.length);
        return /\.(?:jpe?g|png|gif)$/i.test(before) ? "" : suffix;
      });
    original.search = "";
    add(original.toString(), Number.MAX_SAFE_INTEGER, "drupal_original_asset");
  }
...
export function collectProductImageCandidates({
  ...
  const add = ({
    ...
    if (!resolved) return;
    const effectiveDeclaredWidth = Math.max(
      Number(declaredWidth || 0),
      inferDeclaredWidthFromImageUrl(resolved)
    );
    ...
    candidates.push({
      ...
      declaredWidth: effectiveDeclaredWidth,
  ...
  while ((imageMatch = imagePattern.exec(sourceText)) !== null) {
    ...
    // Parent/container attributes appear before an <img>. Do not include a
    // broad trailing slice here: a following recommendations section must not
    // make the current main-gallery image look like a related-product image.
    const context = sourceText.slice(
      Math.max(0, imageMatch.index - 650),
      imageMatch.index + tag.length
    );
...
export function selectMostLikelyMainProductGalleryImage(candidates = []) {
  const originalCandidates = (candidates || []).filter(
    (candidate) =>
      candidate?.url &&
      !/^derived:/i.test(String(candidate?.source || ""))
  );
  const negativeContextPattern =
    /(?:recommend|similar|related|recently|cross[-_ ]?sell|upsell|logo|favicon|icon|sprite|badge|avatar|banner|background|swatch|judgeme|judge\.me|loox|yotpo|stamped(?:\.io)?|trustpilot|bazaarvoice|powerreviews|review[-_ /]?(?:image|photo|media|gallery)|customer[-_ /]?(?:image|photo|review|media)|user[-_ ]?generated|\bugc\b|testimonial)/i;
  const likelyMainGalleryCandidates = originalCandidates.filter(
    (candidate) =>
      Number(candidate?.roleScore || 0) >= 90 &&
      !negativeContextPattern.test(
        `${candidate?.source || ""} ${candidate?.context || ""}`
      )
  );
  const metadataFallbackCandidates = originalCandidates.filter((candidate) =>
    /^(?:selected_product_image|product_json_ld|og:image)$/i.test(
      String(candidate?.source || "")
    )
  );
  const selectionPool = likelyMainGalleryCandidates.length
    ? likelyMainGalleryCandidates
    : metadataFallbackCandidates;

  const roleTier = (candidate) => {
    const score = Number(candidate?.roleScore || 0);
  ...
  return [...selectionPool].sort((left, right) => {
    const tierDelta = roleTier(right) - roleTier(left);
    if (tierDelta !== 0) return tierDelta;
    const widthDelta =
      Number(right?.declaredWidth || 0) -
      Number(left?.declaredWidth || 0);
    if (widthDelta !== 0) return widthDelta;
    return Number(right?.roleScore || 0) - Number(left?.roleScore || 0);
  })[0] || null;
}
...
export function isViableProductImageIdentityAnchor(metadata) {
  const width = Number(metadata?.width || 0);
  const height = Number(metadata?.height || 0);
  return (
    Math.max(width, height) >= PRODUCT_IMAGE_IDENTITY_ANCHOR_MIN_LONG_SIDE &&
    width * height >= PRODUCT_IMAGE_IDENTITY_ANCHOR_MIN_AREA
  );
}
...
export async function selectLargestVerifiedProductImage({
  ...
  const accepted = inspected.filter(
    (candidate) =>
      ...
      candidate.width > 0 &&
      candidate.height > 0 &&
      !isReviewOrUserGeneratedProductImageCandidate(candidate)
  );
  const rejected = inspected.filter(
    (candidate) =>
      candidate.rejected ||
      isReviewOrUserGeneratedProductImageCandidate(candidate)
  );
  ...
  const primaryInspected =
    ...
    null;
  const primaryIdentityAnchor = isViableProductImageIdentityAnchor(
    primaryInspected
  )
    ? primaryInspected
    : null;
  ...
  for (const candidate of accepted) {
    candidate.samePrimaryAsset =
      Boolean(primaryIdentityAnchor && primaryKey) &&
      candidate.assetKey === primaryKey;
    candidate.fingerprintSimilarity =
      primaryIdentityAnchor?.fingerprint && candidate?.fingerprint
        ? productImageFingerprintSimilarity(
            primaryIdentityAnchor.fingerprint,
            candidate.fingerprint
          )
        : 0;
    const trustedWithoutPrimary =
      !primaryIdentityAnchor &&
      candidate.roleScore >= 105 &&
      /^(?:product_json_ld|rendered:)/i.test(
        String(candidate?.source || "")
      );
    candidate.identityVerified =
      candidate.samePrimaryAsset ||
      candidate.fingerprintSimilarity >= 0.9 ||
      trustedWithoutPrimary;
    candidate.identityMethod = candidate.samePrimaryAsset
      ? "same_primary_asset"
      : candidate.fingerprintSimilarity >= 0.9
        ? "visual_match_to_primary"
        : trustedWithoutPrimary
      ...
      ? [
          {
            ...
            identityMethod: "existing_primary_identity_anchor",
          },
        ]
      : [];
  ...
  return {
    selected,
    inspected: accepted,
    verifiedCandidates: selectionPool,
    rejected,
