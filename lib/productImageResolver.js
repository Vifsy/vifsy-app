...
export async function selectLargestVerifiedProductImage({
  ...
  const nonPrimaryFallback = accepted.filter(
    (candidate) => !primaryKey || candidate.assetKey !== primaryKey
  );
  const selectionPool = verified.length
    ? verified
    : primaryIdentityAnchor
      ? [primaryIdentityAnchor]
      : nonPrimaryFallback.filter((candidate) => candidate.roleScore >= 90);
  const safePool = selectionPool.length
    ? selectionPool
    : nonPrimaryFallback.length
      ? nonPrimaryFallback
      : [primaryInspected || accepted[0]];
