...
export async function sampleRemoteVideoFrames({
  ...
  try {
    ...
    const requestedTimes = Array.isArray(timesSeconds) && timesSeconds.length
      ? timesSeconds
          .map((value) => Number(value))
          .filter((value) => Number.isFinite(value))
          .map((value) => Math.max(0.05, Math.min(duration - 0.02, value)))
      : fractions.map((fraction) =>
          Math.max(0.05, Math.min(duration - 0.08, duration * Number(fraction)))
        );
