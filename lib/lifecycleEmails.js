...
import { resolveLocaleFromUserMetadata } from "./userAppLocale.js";
...
export async function sendLifecycleEmail({
  ...
  // v144.111: the user's saved app language is authoritative for every
  // customer-facing lifecycle email. The caller-provided locale is only a
  // fallback for older accounts that do not yet have app_locale metadata.
  const resolvedLocale = resolveLocaleFromUserMetadata(
    authUser?.user?.user_metadata || {},
    resolveBestServerLocale({ languageCandidates: [locale] })
  );
  ...
  try {
    ...
    if (isCalendarUpdated) {
      ...
      intro = t("emails.analysisCompleted.intro", { brand: safeBrandName });
