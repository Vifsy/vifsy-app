...
export function buildVideoOverlayEdit({
  ...
  if (!videoUrl || !textOverlayUrl) {
    throw new Error("Kling advertising post-process needs a video URL and transparent text overlay URL");
  }

  const duration = Math.max(3, Math.min(10, Number(durationSeconds) || 6));
  const overlayStart = Math.max(0, Math.min(duration - 0.8, Number(overlayStartSeconds) || 0));
  const overlayLength = Math.max(0.8, duration - overlayStart);
  ...
  return {
    ...
    output: {
      ...
      poster: {
        capture: Math.min(duration - 0.2, Math.max(0.1, overlayStart + 0.4)),
      },
    },
  };
}
