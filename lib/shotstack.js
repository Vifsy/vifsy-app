...
export function buildProductPushEdit({
  ...
  const safeWidth = Math.max(
    280,
    Math.min(920, Math.round(Number(productWidth) || 760))
  );
  const safeHeight = Math.max(
    280,
    Math.min(920, Math.round(Number(productHeight) || 860))
  );
  const canvasWidth = 1080;
  const canvasHeight = 1920;
  const productLeft = Math.round((canvasWidth - safeWidth) / 2);
  const productTop = 255;
  const productCenterX = productLeft + safeWidth / 2;
  const productCenterYFromTop = productTop + safeHeight / 2;
  const productOffsetX = Number(
    ((productCenterX - canvasWidth / 2) / canvasWidth).toFixed(6)
  );
  // Shotstack's positive Y offset moves the clip upwards from canvas center.
  const productOffsetY = Number(
    ((canvasHeight / 2 - productCenterYFromTop) / canvasHeight).toFixed(6)
  );
  const tracks = [];
  ...
  // Keep the HTML5 viewport content-sized. Shotstack's current renderer can fail
  // when a small animated overlay is embedded inside a full 1080x1920 HTML5 clip.
  // Placement on the 1080x1920 canvas is handled by the clip offset instead.
  const productCss = [
    `html,body{margin:0;padding:0;width:${safeWidth}px;height:${safeHeight}px;overflow:hidden;background:transparent}`,
    `.stage{position:relative;width:${safeWidth}px;height:${safeHeight}px;overflow:hidden;background:transparent}`,
    `#product-motion{width:${safeWidth}px;height:${safeHeight}px;transform-origin:50% 50%;will-change:transform}`,
  ...
  const productJs = [
    ...
    `tl.to('#product-motion',{scale:1,duration:${duration - half},ease:'sine.inOut'});`,
  ].join("");
