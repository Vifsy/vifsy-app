...
const DEFINITIONS = {
  website_item: { usesWebsiteContent:true, prompt:"Use the website URL from the brand profile. Identify one concrete, verified product with a usable product image from the website. Create a social media post that promotes that specific product in a helpful, trustworthy and sales-focused way. Use only information that clearly appears on the website. Do not invent prices, discounts, guarantees, features or availability.", imagePrompt:"Use the verified website product image without adding product names, prices, labels or other text. Preserve the real product identity." },
