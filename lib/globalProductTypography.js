...
export function normalizeIntlLocale(value, fallback = "und") {
  const raw = String(value || "").trim().replace(/_/g, "-");
  if (!raw) return fallback;
  try {
    return Intl.getCanonicalLocales(raw)[0] || fallback;
  } catch {
    // Brand/content language settings may contain human labels or legacy
    // underscore locales. Segmentation is a local rendering aid only, so an
    // invalid locale must never be allowed to fail an otherwise valid post.
    return fallback;
  }
}
...
function getGraphemes(value, locale = "und") {
  ...
  if (typeof Intl?.Segmenter === "function") {
    ...
    return Array.from(new Intl.Segmenter(safeLocale, { granularity: "grapheme" }).segment(text), (entry) => entry.segment);
  }
...
function segmentWords(value, locale = "und") {
  ...
  if (typeof Intl?.Segmenter === "function") {
    const safeLocale = normalizeIntlLocale(locale);
    const segments = Array.from(new Intl.Segmenter(safeLocale, { granularity: "word" }).segment(text));
...
export function layoutProductTitle(value, { maxWidth = 350, maxHeight = 176, languageHint = "" } = {}) {
  const sizes = [46, 42, 38, 34, 31, 28, 25, 22];
